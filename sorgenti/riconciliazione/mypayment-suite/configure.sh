...
_valSed(){
	VAL_SED="$1"
	VAL_SED=$(echo "$VAL_SED"|sed -e 's/\//\\\//g')
	VAL_SED=$(echo "$VAL_SED"|sed -e 's/\./\\./g')
	VAL_SED=$(echo "$VAL_SED"|sed -e 's/\@/\\@/g')
	VAL_SED=$(echo "$VAL_SED"|sed -e 's/\-/\\-/g')
	VAL_SED=$(echo "$VAL_SED"|sed -e 's/\[/\\[/g')
...
_build(){
	f=`echo "$CONF_FILE" | grep "/"`
	#echo "CONF_FILE=[$CONF_FILE]"
	case $f in '');;*) echo -e "\nERRORE:\n\tcarattere '/' non consentito nel nome file\n";exit;;esac
	DIST=`echo "$CONF_FILE"|sed -e 's/configure\.//g'|sed -e 's/\.txt$//g'`
	case $DIST in
	'')
...
_integrityCheck(){
	...
	checkStatus=0;k=0;
	PS_ARR=""
	for line in `grep -v "^#" $CONF_FILE|tr -d " "|grep -v "^$"`
	do
		PS=`echo "$line"|awk -F '=' '{print $1}'`
...
_search(){
	...
	*);;
	...
	#
	i=1;
	tot=`grep -v "^#" $CONF_FILE|grep -v "^$"|wc -l|awk '{print $1}'`
	for line in `grep -v "^#" $CONF_FILE|grep -v "^$"|tr -d " "`
	do
		PS=`echo "$line"|awk -F '=' '{print $1}'`
		VAL=`echo "$line"|awk -F '=' '{print $2}'`
		...
		for dir in $DIRS
		do
			#echo "**** grep -rl -w \"$PS\" dist/$DIST/$dir/* ***"
			FILES=`grep -rl -w "$PS" build/$DIST/$dir/*`
			for file in $FILES
			do
				echo "sed -i 's/$PS/$VAL_SED/g' $file" >> .tmp/$PS
			done
		done
		i=$((i+1))
	done
...
_check(){
	...
	FILES=`grep -lr "_TAG_" build/$DIST/*`
