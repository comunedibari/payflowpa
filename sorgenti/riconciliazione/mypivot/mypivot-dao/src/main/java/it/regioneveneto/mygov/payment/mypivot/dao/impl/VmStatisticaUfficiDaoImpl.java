 ...
 */
@Repository
public class VmStatisticaUfficiDaoImpl implements VmStatisticaUfficiDao {

	private static final Logger logger = Logger.getLogger(VmStatisticaUfficiDaoImpl.class);
	...
	public CruscottoIncassiDto getTotaliRipartitiPerUfficiByAnno(Long enteId, Integer year, List<String> codTipiDovuto) throws Exception{
		try {
			logger.debug("DAO :: STATISTICA :: TOTALI RIPARTITI per UFFICI :: PARAMETRI DI RICERCA[enteId:" + enteId + ", year:" + year + ", " +
						 "codTipiDovuto:" + codTipiDovuto + "] :: START");
			...
			if (enteId == null || year == null || codTipiDovuto == null || codTipiDovuto.isEmpty()) {
				 ...
				 */
				String errMsg = "Impossibile avviare la ricerca statistica 'TOTALI RIPARTITI per UFFICI'. Parametri obbligatori assenti o non corretti: " + 
			...
			String sql = "SELECT " +
						 ...
						 "GROUP BY " + 
						 		"mygov_ente_id, anno, cod_uff, de_uff " +
						 "ORDER BY " +
						 		"cod_uff ";
			...
			for (Object[] row : rows) {
				 ...
				 */
				String _codUfficio = (String) row[2]; 		String _deUfficio = (String) row[3];
				/**
				 * Get Importi
				 */
				...
				item.setDesc(StringUtils.hasText(_deUfficio) ? _deUfficio : _codUfficio);
				//
				item.setCodice(_codUfficio);
				//
			...
			logger.debug("DAO :: STATISTICA :: TOTALI RIPARTITI per UFFICI :: END");
			
			return dto;
		}catch(Exception e) {
			logger.error("DAO :: STATISTICA :: TOTALI RIPARTITI per UFFICI :: ERROR ", e);
	...
	public CruscottoIncassiDto getTotaliRipartitiPerUfficiByAnnoMese(Long enteId, Integer year, Integer month, List<String> codTipiDovuto) throws Exception{
		try {
			logger.debug("DAO :: STATISTICA :: TOTALI RIPARTITI per UFFICI :: PARAMETRI DI RICERCA[" + 
			...
			String sql = "SELECT " +
						 ...
						 "GROUP BY " + 
						 		"mygov_ente_id, anno, mese, cod_uff, de_uff " +
						 "ORDER BY " +
								"cod_uff ";
			...
			for (Object[] row : rows) {
				 ...
				 * Get codice e descrizione
				 */
				String _codUfficio = (String) row[3]; 		String _deUfficio = (String) row[4];
				/**
				 * Get Importi
				 */
	...
	public CruscottoIncassiDto getTotaliRipartitiPerUfficiByAnnoMeseGiorno(Long enteId, Integer year, Integer month, Integer day, List<String> codTipiDovuto) throws Exception {
		try {
			logger.debug("DAO :: STATISTICA :: TOTALI RIPARTITI per UFFICI :: PARAMETRI DI RICERCA[" + 
						 "enteId:" + enteId + ", year:" + year + ", month:" + month  + ", day:" + day + ", codTipiDovuto:" + codTipiDovuto + "] :: START");
			...
			String sql = "SELECT " + 
						 ...
						 "GROUP BY " + 
						 		"mygov_ente_id, anno, mese, giorno, cod_uff, de_uff " +
						 "ORDER BY " +
								"cod_uff ";
			...
			for (Object[] row : rows) {
				 ...
				 * Get codice e descrizione
				 */
				String _codUfficio = (String) row[4]; 		String _deUfficio = (String) row[5];
				/**
				 * Get Importi
				 */
