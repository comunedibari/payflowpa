...
import it.regioneveneto.mygov.payment.mypivot.domain.dto.cruscottoIncassi.CruscottoIncassiDovutiInputDto;
...
public class CruscottoIncassiDovutiController extends BaseController {
	...
	@ApiOperation(value = "get statistiche pagamenti aggregati per Tipo di Dovuto", notes = ApiDescription.DESCR_GET_STATISTICHE_PER_DOVUTI, response = SingleDataResponse.class)
	@GetMapping(value = "/totaliRipartitiPerDovuto")	
	public ResponseIF totaliRipartitiPerDovuto(

			@RequestBody CruscottoIncassiDovutiInputDto cruscottoIncassiInput
		...
		log.debug("RICERCA :: STATISTICHE :: RIPARTITI per DOVUTI :: GET :: Utente[codFedUserId: " + utenteTO.getCodFedUserId() + ", cruscottoIncassiInput: " + cruscottoIncassiInput.toString() + "] :: START");
		...
		try{
			...
			if(cruscottoIncassiInput.getAnnoCheck() != null && cruscottoIncassiInput.getAnnoCheck())
				 ...
				 * Filtro per Anno
				 */
				result = vmStatisticheService.getTotaliRipartitiPerTipiDovutoByAnno(ente.getId(), Integer.parseInt(cruscottoIncassiInput.getAnno()), activeOperatoreEnteTdAsString);
			else
				if(cruscottoIncassiInput.getAnnoMeseCheck() != null && cruscottoIncassiInput.getAnnoMeseCheck()){
					...
					/* */
					result = vmStatisticheService.getTotaliRipartitiPerTipiDovutoByAnnoMese(ente.getId(), cal.get(Calendar.YEAR), cal.get(Calendar.MONTH)+1, activeOperatoreEnteTdAsString);
				}
				else
					if(cruscottoIncassiInput.getAnnoMeseGiornoCheck() != null && cruscottoIncassiInput.getAnnoMeseGiornoCheck()){
						...
						/* */
						result = vmStatisticheService.getTotaliRipartitiPerTipiDovutoByAnnoMeseGiorno(ente.getId(), cal.get(Calendar.YEAR), cal.get(Calendar.MONTH)+1, cal.get(Calendar.DAY_OF_MONTH), activeOperatoreEnteTdAsString);
					}

		}catch(Exception e){
			log.error("RICERCA :: STATISTICA :: RIPARTITI per DOVUTI :: Utente[codFedUserId: " + utenteTO.codFedUserId + "] :: ERRORE ", e);
			messagesDto.getInformationMessages().add(new DynamicMessageDto("mypivot.statistiche.errore.ricerca"));
			response.setMessages(messagesDto);
			return response;
		}

		log.debug("RICERCA :: STATISTICHE :: RIPARTITI per DOVUTI :: GET :: Utente[codFedUserId: " + utenteTO.getCodFedUserId() + ", cruscottoIncassiInput: " + cruscottoIncassiInput.toString() + "] :: END");
	...
	public static void initializeFilter(CruscottoIncassiDovutiInputDto statisticheFilter, Boolean flgTesoreria) {
		/**
