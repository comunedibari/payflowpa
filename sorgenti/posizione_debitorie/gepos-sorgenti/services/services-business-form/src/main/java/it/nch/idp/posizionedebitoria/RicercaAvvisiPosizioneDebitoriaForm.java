...
public class RicercaAvvisiPosizioneDebitoriaForm extends ValidatorForm   {
	...
	public Boolean checkRequiredFieldsStorico() {
		if (
				   StringUtils.isEmpty(iuv) 
				&& StringUtils.isEmpty(this.cfIntestatario) 
				&& StringUtils.isEmpty(idPendenzaEnte)
				&& StringUtils.isEmpty(idEnte))
			return false;
		return true;
	}
	
	public Boolean checkRulesStoricoFields() {
		return !StringUtils.isEmpty(idEnte) 
				&&
				(!StringUtils.isEmpty(iuv) 
				|| !StringUtils.isEmpty(this.cfIntestatario) 
				|| !StringUtils.isEmpty(idPendenzaEnte));
