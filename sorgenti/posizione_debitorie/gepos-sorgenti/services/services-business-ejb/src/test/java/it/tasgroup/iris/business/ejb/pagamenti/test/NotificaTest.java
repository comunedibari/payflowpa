...
public class NotificaTest {
	...
	@Test
    public void testEseguitoSBF() throws Exception {
		NotificaLocal bean = (NotificaLocal) context.lookup("java:global/services-business-ejb/NotificaPagamentoBusiness");
		
		NotificaPagamentoDTO dto = new NotificaPagamentoDTO();
		...
		dto.setTestata(test);
		dto.setStatoNotifica(EnumTipoNotifica.ESEGUITO_SBF.getChiave());
		dto.setCodiceTransazione("50d9612a-b572-42c0-b7fe-3e744a2ccf58");
		EsitoOperazionePagamentoDTO  result = bean.notifica(dto); 
		assertEquals(EnumBusinessReturnCodes.OK.getChiave(), result.getCodice());
	}

	@Test
    public void testEseguito() throws Exception {
		NotificaLocal bean = (NotificaLocal) context.lookup("java:global/services-business-ejb/NotificaPagamentoBusiness");
		...
		test.setSession(sess);
		dto.setTestata(test);
		dto.setStatoNotifica(EnumTipoNotifica.ESEGUITO.getChiave());
		dto.setCodiceTransazione("50d9612a-b572-42c0-b7fe-3e744a2ccf58");
		EsitoOperazionePagamentoDTO  result = bean.notifica(dto); 
		assertEquals(EnumBusinessReturnCodes.OK.getChiave(), result.getCodice());
	}
