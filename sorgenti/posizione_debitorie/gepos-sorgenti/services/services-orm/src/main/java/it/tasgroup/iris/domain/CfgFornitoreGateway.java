...
@NamedQuery(
		name="getByBundleKey",
		query="select cf from CfgFornitoreGateway cf "+
			  "where cf.bundleKey =:inBundleKey"),
@NamedQuery(
		name="getCfgFornitoreGatewayAll",
		query="select cf from CfgFornitoreGateway cf " )
}


)
public class CfgFornitoreGateway extends BaseIdEntity {
...
//	private Set<CfgGatewayPagamento> cfgGatewayPagamenti;
	...
	@Id
	@GeneratedValue(strategy=GenerationType.AUTO,generator="cfg_fornitore_gateway_pk_gen")	
	@SequenceGenerator(
		    name="cfg_fornitore_gateway_pk_gen",
