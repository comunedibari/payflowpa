...
public class DistintePagamentoDaoTest  {
    ...
    @Test	
	public void testCreaDPCampiObbligatori() {
	
		try {
			GestioneFlussiDao toTest = (GestioneFlussiDao) context.lookup("java:global/services-iris-persistence-ejb/GestioneFlussiDaoService");
			DistintePagamentoDTO dto = new DistintePagamentoDTO();
			
			// campi obbligatori
			dto.setStato(EnumStatoDP.IN_CORSO.getChiave());
			dto.setImporto(new BigDecimal("0.00"));
			dto.setImportoCommissione(new BigDecimal("0.00"));
			dto.setDataCreazione(new Timestamp(System.currentTimeMillis()));
			dto.setUtenteCreatore("FBRMNL75M45H901U");
			dto.setNumDisposizioni(new Integer(0));
//			dto.setCodCanale(EnumCanale.IRIS.getChiave());
//			dto.setCodDistinta(EnumDistinta.DELEGA_RID.getChiave());
			
			String uuid = UUID.randomUUID().toString();
			System.out.println("uuid = " + uuid);

			dto.setCodTransazione(uuid.substring(0, 34));
			dto.setOperatorUsername("FBRMNL75M45H901U");
			
			GestioneFlussi dp = toTest.createDP(dto);
		    System.out.println("test - createDP: id: "+dp.getId());
