...
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.CtInformativaDetail;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.CtInformativaMaster;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.CtInformativaPSP;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.CtInformazioniServizio;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.CtListaInformativePSP;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.ObjectFactory;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.StCodiceLingua;
import it.tasgroup.ndpmodel.nodopagamentispc.payloads.listainformativepsp.v1012mybk.StTipoVersamento;

public class CatalogoPspV1012MYBKDelegator implements ICatalogoPspDelegator {

	private final Log logger = LogFactory.getLog(this.getClass());
	private int catSize;

	@Override
	public void aggiornaGatewayPagamento(List<CfgGatewayPagamento> gatewayAttuali, Object listaInformativePSPObj,
		...
		for (CtInformativaPSP infoPSP : listaInformativePSPType.getInformativaPSP()) {
			for (CtInformativaDetail informativaPspDetail : infoPSP.getListaInformativaDetail()
					.getInformativaDetail()) {

				String bundleKey = infoPSP.getIdentificativoPSP() + "-" + informativaPspDetail.getIdentificativoCanale()
						+ "-" + informativaPspDetail.getTipoVersamento();

				// N.B. uso remove cosi' i rimanenti della map sono tutti i
				...
				if (cfgGateway == null) {
					...
					logger.info("Psp non presente in banca dati. INSERT " + cfgGateway.getBundleKey() + "::"
							+ cfgGateway.getApplicationId() + ":" + cfgGateway.getSystemId());

					if (isGestioneAutomatica) {
//						Timestamp dataInizioValidita = new Timestamp(infoPSP.getInformativaMaster()
//								.getDataInizioValidita().toGregorianCalendar().getTimeInMillis());
						cfgGateway.setStato(NDPConstants.ACTIVE);// setStato(dataInizioValidita.after(NodoPagamentiUtils.now())
																	// ? NDPConstants.TO_ACTIVATE :
																	// NDPConstants.ACTIVE);
					} else {
						// intanto inserisco il record disattivo. Lo attivero' in seguito MANUALMENTE
					...
					//
					mergeCfgGatewayPagamento(cfgGateway, infoPSP, informativaPspDetail, manager);
					logger.info("Psp gia' presente in banca dati. UPDATE " + cfgGateway.getBundleKey() + "::"
							+ cfgGateway.getApplicationId() + ":" + cfgGateway.getSystemId() + " - id: "
							+ cfgGateway.getId());
...
//								.getDataInizioValidita().toGregorianCalendar().getTimeInMillis());
						...
						// nada. infatti la gestione e' manuale.
					}
					cfgGateway.setStRiga(NDPConstants.ST_RIGA_V);
		...
		for (String key : mappaGateway.keySet()) {
			...
			if ("AGID_01".equals(cfgGateway.getSystemId())) {
				logger.info("DISATTIVAZIONE GATEWAY. NON DISATTIVO IL GATEWAY AGID_01 " + cfgGateway.getBundleKey()
						+ "::" + cfgGateway.getApplicationId() + ":" + cfgGateway.getSystemId() + " - id: "
						+ cfgGateway.getId());
			} else {
				logger.info("DISATTIVAZIONE GATEWAY. UPDATE " + cfgGateway.getBundleKey() + "::"
						+ cfgGateway.getApplicationId() + ":" + cfgGateway.getSystemId() + " - id: "
						+ cfgGateway.getId());
				cfgGateway.setStato(NDPConstants.DEACTIVE);
	...
	private void mergeCfgGatewayPagamento(CfgGatewayPagamento cfgGateway, CtInformativaPSP informativaPsp,
			CtInformativaDetail informativaPspDetail, EntityManager manager) {
		...
		CtInformazioniServizio infoServizioByLanguage = getInformazioniServizioByLanguage(informativaPspDetail,
				StCodiceLingua.IT);

		Timestamp dataInizioValidita = new Timestamp(
				informativaMaster.getDataInizioValidita().toGregorianCalendar().getTimeInMillis());
		Timestamp dataFineValidita = new Timestamp(new GregorianCalendar(2099, 1, 1, 0, 0).getTimeInMillis());
		Timestamp dataPubblicazione = new Timestamp(
				informativaPsp.getInformativaMaster().getDataPubblicazione().toGregorianCalendar().getTimeInMillis());

		cfgGateway.setBundleKey(informativaPsp.getIdentificativoPSP() + "-"
				+ informativaPspDetail.getIdentificativoCanale() + "-" + informativaPspDetail.getTipoVersamento());
		cfgGateway.setDescrizione(infoServizioByLanguage.getDescrizioneServizio());
		// N.B. guarda metodo chiamante. lo stato viene settato in modo differente a
		// seconda che si decida o meno
		// una gestione "manuale" dello stato
		// cfgGateway.setStato(dataInizioValidita.after(NodoPagamentiUtils.today()) ?
		// NDPConstants.TO_ACTIVATE : NDPConstants.ACTIVE);
		cfgGateway.setStRiga(NDPConstants.ST_RIGA_V);
		...
		cfgGateway.setCfgFornitoreGateway(
				manager.find(CfgFornitoreGateway.class, EnumFornitorePagamento.NODO_PAGAMENTI_SPC.getKey()));

		if (informativaPspDetail.getModelloPagamento() == 4) {
			cfgGateway.setCfgDocumentoPagamento(
					manager.find(CfgDocumentoPagamento.class, EnumDocumentoPagamento.BOLLETTINO_NDP.getKey()));
		} else {
		...
		if (listaCfgCommissioni != null && listaCfgCommissioni.size() > 0) {
			...
			CfgCommissionePagamento cfgCommissione = listaCfgCommissioni.iterator().next(); // Assert: Ci deve essere
																							// sempre uno ed un solo
			...
			nuovaCfgCommissione.setCfgTipoCommissione(
					manager.find(CfgTipoCommissione.class, EnumTipoCommissione.GESTIONE_ESTERNA.getKey()));
	...
	private CtInformazioniServizio getInformazioniServizioByLanguage(CtInformativaDetail infoPspDetail,
			StCodiceLingua language) {
