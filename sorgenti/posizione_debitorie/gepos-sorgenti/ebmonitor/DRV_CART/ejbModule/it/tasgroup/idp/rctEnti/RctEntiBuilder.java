 ...
 *
 */
public class RctEntiBuilder {
 
	private static final String RCT_ENTE_STG = "it/tasgroup/idp/rctEnti/rendicontazioneEnti.stg";
	private static final String RCT_ENTE_SPONTANEO_STG = "it/tasgroup/idp/rctEnti/rendicontazioneEntiSpontaneo.stg";
	 ...
	 * @throws Exception 
	 */
	public String creaRctEntePend(RendicontazioneEntiModel rctEntiModel) throws Exception {
...
//		NotifichePagamentoModel notifichePagamentoModel = new NotifichePagamentoModel();
		...
		try {
			...
			Reader rdr = ResourceLoaders.CLASSPATH.loadReader(RCT_ENTE_STG);
			TemplateWriter tw = new TemplateWriter(rdr, buf);
			BaseTemplateModel tm = new BaseTemplateModel();
			
			tm.set("rendicontoEntiModel", rctEntiModel);				
			tw.write("templateRendicontazioneEnti", tm);			
...
//		logger.info( this.getClass().getSimpleName() + " NotificaPagamento, messaggio = " + output.toString() );
	 ...
	 * @throws Exception 
	 */
	public String creaRctEnteSpontaneo(RendicontazioneEntiModel notifichePagamentoModel) throws Exception {
...
//		NotifichePagamentoModel notifichePagamentoModel = new NotifichePagamentoModel();
		...
		try {
			...
			output = new ByteArrayOutputStream();
			BufferedOutputStream buf = new BufferedOutputStream(output);
			Reader rdr = ResourceLoaders.CLASSPATH.loadReader(RCT_ENTE_SPONTANEO_STG);
			TemplateWriter tw = new TemplateWriter(rdr, buf);
			BaseTemplateModel tm = new BaseTemplateModel();
