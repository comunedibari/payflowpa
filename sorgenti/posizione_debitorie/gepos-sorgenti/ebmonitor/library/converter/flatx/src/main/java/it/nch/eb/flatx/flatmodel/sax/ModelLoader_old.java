...
import it.nch.eb.flatx.flatmodel.conversioninfo.XPathPositionStringFunction;
 ...
 * @author gdefacci
 */
public class ModelLoader_old implements ActivableLeafElementHandler {
	
	private static final org.slf4j.Logger log = ResourcesUtil.createLogger(ModelLoader_old.class);
	...
	private final Map/*<XPathPosition, List<XPathPositionEffect>>*/ startElementXPathEffects = new TreeMap(); 
	private final Map/*<XPathPosition, List<XPathPositionEffect>>*/ endElementXPathEffects = new TreeMap();
	
	private boolean inside = false;
	
	public ModelLoader_old() {
		this((Converter)null);
	}
	
	public ModelLoader_old(Converter textcnv) {
		this(textcnv, new TreeSet());
	}
	
	public ModelLoader_old(Set/*<BaseXPathPosition>*/ topLevelDefinitions) {
		this(null, topLevelDefinitions);
	}
	
	private ModelLoader_old(Converter textcnv, Set/*<BaseXPathPosition>*/ topLevelDefinitions) {
...
//		if (!xpathsMap.isEmpty()) xpathsMap.enterScope();
	...
	protected void execXPathEffects(XPathPosition pos, Map/*<XPathPosition, List<XPathsPositionEffect>>*/ effects) {
		List/*<XPathsPositionEffect>*/ efctsList = (List) effects.get(pos.getUnindexed());
		if (efctsList!= null && efctsList.size()>0) {
			for (Iterator it = efctsList.iterator(); it.hasNext();) {
				XPathPositionStringFunction efct = (XPathPositionStringFunction) it.next();
				log.debug("\nposition " + pos + "\nexecuting " + efct);
				efct.apply(pos);
			}
		}
	...
	protected static void addEffect(Map/*<XPathPosition, List<XPathPositionEffect>>*/ effectsMap, 
			XPathPosition pos, 
