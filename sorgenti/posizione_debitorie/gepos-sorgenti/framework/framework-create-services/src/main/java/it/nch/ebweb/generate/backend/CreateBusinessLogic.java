 ...
 */
public class CreateBusinessLogic extends  Creator{
	...
	private void createBL() {
		...
		try {
			...
			if (file.exists()) {
				warn();
				System.out.println("L'Impl esiste gia', non lo sovrascrivo");
				return;
			}
	...
	private void createDao(int type) {
		...
		try {
			...
			if (file.exists()) {
				warn();
				System.out.println("Il Dao esiste gia', non lo sovrascrivo");
				return;
			}
			...
			writer.println("import "+CreatorCostanst.FWK_PACKAGE+".core.StatefulSessionManager;");
		...
		try {
			...
			switch(daoType){
				case 0:
					...
					writer.println("          <value>"+this.service.toLowerCase()+"AuditLoggerInterceptor</value>");
					writer.println("       </list>");
					writer.println("    </property>");
					...
					writer.println("");
					writer.println("  <bean id=\""+this.service.toLowerCase()+"AuditLoggerInterceptor\" class=\"it.nch.iris.business.custom.util.audit.AuditLoggerInterceptor\"/>");
					writer.println("");
					//*****************************
